...
(function () {
  ...
  const TYPE_ALIASES = {
    "attraction": "景点", "景區": "景点", "景区": "景点", "其他": "景点",
    "restaurant": "餐厅", "restaurants": "餐厅", "餐饮": "餐厅", "咖啡": "餐厅", "咖啡馆": "餐厅", "cafe": "餐厅", "coffee": "餐厅",
    "snack": "小吃", "snacks": "小吃", "小食": "小吃", "美食": "小吃",
    "photo": "拍照机位", "photo spot": "拍照机位", "拍摄机位": "拍照机位", "摄影": "拍照机位", "夜景": "拍照机位",
    "park": "公园", "garden": "公园", "绿地": "公园",
    "museum": "博物馆", "gallery": "博物馆", "美术馆": "博物馆",
    "shopping": "购物", "shop": "购物", "商场": "购物",
    "transport": "交通", "station": "交通", "交通站点": "交通", "车站": "交通",
    "hotel": "酒店", "accommodation": "酒店", "住宿": "酒店", "旅馆": "酒店",
    "airport": "机场", "航空站": "机场", "航站楼": "机场",
    "high speed rail": "高铁站", "railway station": "高铁站", "高铁": "高铁站", "火车站": "高铁站"
  };
  const FILTER_GROUPS = {
    "全部": { icon: "▦", types: null },
    "景点": { icon: "🏛️", types: ["景点", "博物馆", "公园"] },
    "美食": { icon: "🍔", types: ["餐厅", "小吃"] },
    "购物": { icon: "🛍️", types: ["购物"] },
    "夜景": { icon: "🌙", types: ["拍照机位"] },
    "住宿": { icon: "🏨", types: ["酒店"] }
  ...
  const dom = {
    ...
    cityPopover: document.getElementById("cityPopover"),
    ...
    routePlanTabs: document.getElementById("routePlanTabs"),
    renameRouteBtn: document.getElementById("renameRouteBtn"),
    ...
    savedRouteList: document.getElementById("savedRouteList"),
    settingsAddRouteBtn: document.getElementById("settingsAddRouteBtn"),
    settingsCurrentCityName: document.getElementById("settingsCurrentCityName"),
  ...
  function updateMapContentSize() {
    ...
    const width = innerWidth <= 720
      ? Math.max(rect.width * 1.35, Math.min(rect.height * 1.15, rect.width * 1.9))
      : rect.width;
  ...
  function setSearchExpanded(open) {
    ...
    dom.searchShell.classList.toggle("open", expanded);
    dom.searchToggle.setAttribute("aria-expanded", String(expanded));
    dom.searchToggle.setAttribute("aria-label", expanded ? "关闭地点搜索" : "打开地点搜索");
    if (!expanded) {
  ...
  function setNavigationCollapsed(collapsed, save = true) {
    ...
    const icon = dom.topbarToggle.querySelector(".topbar-toggle-icon");
    if (icon) icon.textContent = collapsed ? "⌃" : "⌄";
    dom.topbarToggle.querySelector(".topbar-toggle-text").textContent = collapsed ? "显示分类" : "隐藏分类";
    dom.topbarToggle.setAttribute("aria-label", collapsed ? "显示地点分类" : "隐藏地点分类");
    if (save) {
  ...
  function orderedCityIds() {
    const existing = Object.keys(db.cities || {});
    return [
      ...FIXED_CITY_IDS.filter(cityId => existing.includes(cityId)),
      ...existing.filter(cityId => !FIXED_CITY_IDS.includes(cityId))
    ];
  }

  function setCityPopover(open) {
    if (!dom.currentCityBtn || !dom.cityPopover) return;
    dom.currentCityBtn.setAttribute("aria-expanded", String(open));
    dom.cityPopover.hidden = !open;
    dom.cityPopover.classList.toggle("open", open);
  }

  function switchCity(cityId) {
    if (!db.cities?.[cityId]) return;
    if (cityId === db.currentCity) {
  ...
  function renderCitySwitcher() {
    if (!dom.cityPopover) return;
    dom.cityPopover.replaceChildren();
    for (const cityId of orderedCityIds()) {
      const cityData = db.cities[cityId];
      if (!cityData) continue;
      const button = document.createElement("button");
      const current = cityId === db.currentCity;
      button.type = "button";
      button.className = `quick-city-option${current ? " current" : ""}`;
      button.setAttribute("role", "menuitemradio");
      button.setAttribute("aria-checked", String(current));
      button.innerHTML = `<span><b>${escapeHtml(cityData.name)}</b><small>${escapeHtml(cityData.summary || "切换城市地图")}</small></span><span aria-hidden="true">${current ? "✓" : "切换"}</span>`;
      button.addEventListener("click", () => switchCity(cityId));
      dom.cityPopover.appendChild(button);
    }
  }
  ...
  function normalisePlace(cityId, input, placeIndex) {
    ...
    place.photoQuery = String(place.photoQuery || `${place.name} ${layoutFor(cityId)?.title || cityId}`).trim();
    place.photoResolved = Boolean(place.photoResolved && place.images.length && !place.autoCover);
    place.photoAttribution = String(place.photoAttribution || "");
    place.favorite = Boolean(place.favorite);
  ...
  function normaliseRoutePlan(input, index, legacyRoute = [], legacySegments = {}) {
    const plan = isPlainObject(input) ? input : {};
    plan.id = String(plan.id || `route_${index + 1}`);
    plan.name = String(plan.name || `线路 ${index + 1}`);
    plan.route = Array.isArray(plan.route) ? plan.route.map(id => String(id)) : legacyRoute.map(id => String(id));
    plan.segments = isPlainObject(plan.segments) ? plan.segments : deepClone(legacySegments);
    return plan;
  }
  ...
  function normaliseCity(cityId, input) {
    ...
    const legacyRoute = Array.isArray(source.route) ? source.route.map(id => String(id)) : [];
    const legacySegments = isPlainObject(source.segments) ? source.segments : {};
    source.routePlans = Array.isArray(source.routePlans)
      ? source.routePlans.map((plan, index) => normaliseRoutePlan(plan, index))
      : [];
    if (!source.routePlans.length) source.routePlans.push(normaliseRoutePlan(null, 0, legacyRoute, legacySegments));
    const ids = new Set();
    source.routePlans.forEach((plan, index) => {
      let nextId = plan.id;
      while (ids.has(nextId)) nextId = `${plan.id}_${index + 1}`;
      plan.id = nextId;
      ids.add(nextId);
    });
    source.activeRouteId = ids.has(String(source.activeRouteId)) ? String(source.activeRouteId) : source.routePlans[0].id;
    const activePlan = source.routePlans.find(plan => plan.id === source.activeRouteId) || source.routePlans[0];
    source.route = activePlan.route;
    source.segments = activePlan.segments;
    return source;
  }
  ...
  function normaliseDatabase(input, strict = false) {
    ...
    const previousDataVersion = Number(result.meta.version) || 0;
    result.meta.app = "JJTrip";
    result.meta.version = 9;
    ...
    for (const cityId of FIXED_CITY_IDS) {
      ...
      } else if (previousDataVersion < 9 && isPlainObject(result.cities[cityId]) && isPlainObject(SEED.cities[cityId])) {
        const existingIds = new Set((result.cities[cityId].places || []).map(place => String(place?.id || "")));
        for (const seedPlace of SEED.cities[cityId].places || []) {
          if (!existingIds.has(String(seedPlace.id))) result.cities[cityId].places.push(deepClone(seedPlace));
        }
      }
    }
  ...
  function activeRoutePlan(cityData = currentCity()) {
    let plan = cityData.routePlans.find(item => item.id === cityData.activeRouteId);
    if (!plan) {
      plan = cityData.routePlans[0];
      cityData.activeRouteId = plan.id;
    }
    return plan;
  }

  function syncActiveRouteLegacy(cityData = currentCity()) {
    const plan = activeRoutePlan(cityData);
    cityData.route = plan.route;
    cityData.segments = plan.segments;
  }
  ...
  function validRouteEntries() {
    ...
    const routePlan = activeRoutePlan(cityData);
    const placeById = new Map(cityData.places.map(place => [place.id, place]));
    const entries = [];
    routePlan.route.forEach((id, storedIndex) => {
  ...
  function renderMapBase() {
    ...
    if (dom.settingsCurrentCityName) dom.settingsCurrentCityName.textContent = cityData.name;
  ...
  function handleBottomNavigation(target) {
    ...
    if (dom.settings.classList.contains("open")) closeSettings();
    activeNav = target;
    if (target === "home") {
      favoriteOnly = false;
      filterType = "全部";
    ...
    if (target === "favorites") {
      ...
      renderMarkers();
      renderBottomNav();
      const count = currentCity().places.filter(place => place.favorite).length;
      showToast(count ? `已显示 ${count} 个收藏地点` : "还没有收藏地点，可先打开地点详情收藏");
    }
  }

  function renderFilters() {
    const categories = Object.keys(FILTER_GROUPS);
    if (!categories.includes(filterType)) filterType = "全部";
    dom.filterBar.replaceChildren();
    for (const category of categories) {
      const config = FILTER_GROUPS[category];
      const button = document.createElement("button");
      button.type = "button";
      button.className = `filter-chip${filterType === category ? " active" : ""}`;
      button.innerHTML = `<span aria-hidden="true">${escapeHtml(config.icon)}</span><b>${escapeHtml(category)}</b>`;
      button.setAttribute("aria-pressed", String(filterType === category));
  ...
  function placeMatchesSearch(place, query) {
    ...
    const haystack = [place.name, place.type, place.note, place.address, place.photoQuery, ...(place.tags || [])].join(" ").toLocaleLowerCase("zh-CN");
    return haystack.includes(query);
  }

  function filteredPlaces() {
    const query = searchText.toLocaleLowerCase("zh-CN");
    const group = FILTER_GROUPS[filterType] || FILTER_GROUPS["全部"];
    return currentCity().places.filter(place => {
      if (favoriteOnly && !place.favorite) return false;
      if (group.types && !group.types.includes(place.type)) return false;
      return placeMatchesSearch(place, query);
    });
  }

  function searchMatches(limit = 8) {
    const query = searchText.trim().toLocaleLowerCase("zh-CN");
    if (!query) return [];
    return currentCity().places
      .filter(place => placeMatchesSearch(place, query))
      .sort((a, b) => {
        const aStarts = a.name.toLocaleLowerCase("zh-CN").startsWith(query) ? 0 : 1;
        const bStarts = b.name.toLocaleLowerCase("zh-CN").startsWith(query) ? 0 : 1;
  ...
  function renderSearchResults() {
    ...
    const shouldOpen = Boolean(searchText.trim()) && (dom.searchShell.classList.contains("open") || document.activeElement === dom.searchInput);
    ...
    if (!matches.length) {
      ...
      empty.innerHTML = `<span aria-hidden="true">⌕</span><div><b>没有找到地点</b><small>试试地点名称、类型或地址</small></div>`;
      ...
      for (const place of matches) {
        ...
        button.innerHTML = `<span class="search-result-icon${placeIconClass(info.emoji)}" style="--marker-color:${info.color}">${escapeHtml(info.emoji)}</span><span><b>${escapeHtml(place.name)}</b><small>${escapeHtml(place.type)} · ${escapeHtml(place.address || currentCity().name)}</small></span><i aria-hidden="true">›</i>`;
        button.addEventListener("click", () => {
          favoriteOnly = false;
          activeNav = "places";
          filterType = "全部";
    ...
    requestAnimationFrame(() => dom.searchResults.classList.add("open"));
  }
  ...
  function focusPlaceOnMap(place) {
    ...
    const rect = mapViewportRect();
    const size = updateMapContentSize();
    const scale = clamp(Math.max(mapView.scale, 1.35), MAP_MIN_SCALE, MAP_MAX_SCALE);
    const pointX = size.width * place.mapX / 100;
    const pointY = size.height * place.mapY / 100;
    mapView = {
      scale,
      x: rect.width * 0.5 - pointX * scale,
      y: rect.height * 0.46 - pointY * scale
    };
    applyMapView();
  }
  ...
  function resolveMarkerPositions() {
    ...
    const separation = (innerWidth <= 720 ? 48 : 30) / Math.max(0.78, mapView.scale);
    const step = separation + 3 / Math.max(0.78, mapView.scale);
  ...
  function placeCoverDataUri(place) {
    const info = typeInfo(place);
    const title = String(place.name || "地点").slice(0, 18);
    const cityName = currentCity()?.name || "JJTrip";
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="720" viewBox="0 0 1200 720">
      <defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${info.color}"/><stop offset="1" stop-color="#dbeafe"/></linearGradient></defs>
      <rect width="1200" height="720" rx="54" fill="url(#g)"/>
      <circle cx="600" cy="270" r="128" fill="rgba(255,255,255,.88)"/>
      <text x="600" y="318" text-anchor="middle" font-size="118" font-family="Arial, sans-serif">${escapeHtml(info.emoji)}</text>
      <text x="600" y="500" text-anchor="middle" font-size="68" font-weight="800" fill="#18324d" font-family="Arial, sans-serif">${escapeHtml(title)}</text>
      <text x="600" y="575" text-anchor="middle" font-size="34" font-weight="700" fill="#47637f" font-family="Arial, sans-serif">${escapeHtml(cityName)} · JJTrip 地点封面</text>
    </svg>`;
  ...
  async function fetchCommonsPhoto(query) {
    ...
    for (const page of pages) {
      ...
      const source = String(info?.thumburl || info?.url || "");
      if (!source || !mime.startsWith("image/") || /svg|gif/i.test(mime)) continue;
      const metadata = info.extmetadata || {};
      const artist = stripMarkup(metadata.Artist?.value || metadata.Credit?.value || "");
      const license = stripMarkup(metadata.LicenseShortName?.value || metadata.UsageTerms?.value || "");
      const parts = ["Wikimedia Commons", artist, license].filter(Boolean);
      return { source, attribution: parts.join(" · "), pageUrl: String(info.descriptionurl || "") };
    }
    return null;
  }

  async function fetchCommonsNearbyPhoto(place) {
    const lat = finiteNumber(place?.lat);
    const lng = finiteNumber(place?.lng);
    ...
    const response = await fetch(endpoint, { mode: "cors", credentials: "omit" });
    if (!response.ok) throw new Error(`Commons nearby ${response.status}`);
    const data = await response.json();
    const pages = Object.values(data?.query?.pages || {}).sort((a, b) => (a.index || 999) - (b.index || 999));
    for (const page of pages) {
      const info = page.imageinfo?.[0];
      ...
      const artist = stripMarkup(metadata.Artist?.value || metadata.Credit?.value || "");
      const license = stripMarkup(metadata.LicenseShortName?.value || metadata.UsageTerms?.value || "");
      const parts = ["Wikimedia Commons · 附近实景", artist, license].filter(Boolean);
      return { source, attribution: parts.join(" · "), pageUrl: String(info.descriptionurl || "") };
    }
    return null;
  }
  ...
  async function resolvePlacePhoto(place, cityId = db.currentCity) {
    if (!place || (place.images?.length && !place.autoCover) || place.photoAutoDisabled) return false;
    const recentTry = Number(place.photoTriedAt) || 0;
    if (recentTry && Date.now() - recentTry < 24 * 60 * 60 * 1000) return false;
    const requestKey = `${cityId}:${place.id}`;
    if (photoRequests.has(requestKey)) return photoRequests.get(requestKey);
    const task = (async () => {
      place.photoTriedAt = Date.now();
      const cityData = db.cities[cityId];
      const query = String(place.photoQuery || `${place.name} ${cityData?.name || ""}`).trim();
      let result = null;
      try { result = await fetchWikipediaPhoto(query); } catch (_) {}
      if (!result) {
        try { result = await fetchCommonsPhoto(query); } catch (_) {}
      }
      if (!result) {
        try { result = await fetchCommonsNearbyPhoto(place); } catch (_) {}
      }
      if (!result && cityData) {
        try { result = await fetchCommonsPhoto(`${cityData.name} ${place.type}`); } catch (_) {}
      }
      if (!result?.source || (place.images?.length && !place.autoCover)) return false;
      place.images = [result.source];
      place.image = result.source;
      place.autoCover = false;
      place.photoResolved = true;
      place.photoAttribution = result.attribution || "Wikimedia Commons";
      place.photoPageUrl = result.pageUrl || "";
      return true;
    })().finally(() => photoRequests.delete(requestKey));
    photoRequests.set(requestKey, task);
    return task;
  }

  function scheduleCityPhotoResolution() {
    clearTimeout(photoQueueTimer);
    ...
    photoQueueTimer = setTimeout(async () => {
      const cityData = db.cities[cityId];
      if (!cityData || db.currentCity !== cityId) return;
      const pending = cityData.places.filter(place => (!place.images.length || place.autoCover) && !place.photoAutoDisabled).slice(0, 20);
      let cursor = 0;
      let changed = false;
      const worker = async () => {
        while (cursor < pending.length) {
          const place = pending[cursor++];
          if (await resolvePlacePhoto(place, cityId)) {
            changed = true;
            if (db.currentCity === cityId && selectedId === place.id) renderDetail(place);
          }
        }
      };
      await Promise.all([worker(), worker()]);
      if (pending.length || changed) saveDatabase();
    }, 700);
  }

  function previewMarkup(place, info) {
    const storedImages = Array.isArray(place.images) ? place.images : [];
    const images = storedImages.length ? storedImages : [placeCoverDataUri(place)];
    const isFallback = !storedImages.length || place.autoCover;
    const slides = images.map((image, index) => `<figure class="gallery-slide place-preview" role="group" aria-label="第 ${index + 1} 张，共 ${images.length} 张">
      ...
      <img src="${escapeHtml(image)}" alt="${escapeHtml(place.name)}地点图片 ${index + 1}" ${index ? 'loading="lazy"' : ""}>
      ${index === 0 ? `<figcaption>${escapeHtml(isFallback ? "地点封面 · 联网后自动匹配实景图" : place.photoAttribution || "地点实景图")}</figcaption>` : ""}
    </figure>`).join("");
    ...
    return `<div class="place-gallery${isFallback ? " auto-cover" : ""}" data-gallery-count="${images.length}"><div class="gallery-track" tabindex="0" aria-label="${escapeHtml(place.name)}照片，可左右滑动">${slides}</div>${controls}</div>`;
  }
  ...
  function readOnlyDetails(place) {
    ...
    return `<div class="info-grid info-grid-compact">
  ...
  function editDetails(place) {
    ...
    return `<div class="editor-banner"><span class="editor-banner-mark" aria-hidden="true">✦</span><div><b>编辑地点</b><small>修改内容会自动保存在这台设备</small></div><span class="autosave-badge">自动保存</span></div>
      ...
      <section class="editor-section icon-editor" aria-labelledby="iconEditorTitle">
        ...
        <div class="icon-shortcut-group"><b>住宿与交通快捷图标</b><small>酒店、机场和高铁站可直接手动选择</small><div class="icon-choice-grid" role="group" aria-label="住宿与交通快捷图标">${travelIconButtons}</div></div>
      ...
      <section class="editor-section" aria-labelledby="manageEditorTitle">
        ...
        <div class="danger-zone"><div><b>删除这个地点</b><small>同时会从全部规划线路和交通连接中移除</small></div><button class="btn-danger" id="deletePlaceBtn" type="button">删除地点</button></div>
      </section>`;
  }

  function renderDetail(place) {
    const info = typeInfo(place);
    const inPlan = activeRoutePlan().route.includes(place.id);
    ...
    dom.detailPanel.innerHTML = `<button class="panel-close" id="closeDetailBtn" type="button" aria-label="关闭地点详情" title="关闭">×</button>
      ...
      <div class="actions detail-primary-actions">
        <button class="btn favorite-toggle${place.favorite ? " active" : ""}" id="favoriteToggleBtn" type="button" aria-pressed="${place.favorite}">${place.favorite ? "♥ 已收藏" : "♡ 收藏地点"}</button>
        ${inPlan ? `<span class="plan-state">已加入“${escapeHtml(activeRoutePlan().name)}”</span><button class="btn-danger" id="planToggleBtn" type="button">移出当前线路</button>` : `<button class="btn-primary" id="planToggleBtn" type="button">加入“${escapeHtml(activeRoutePlan().name)}”</button>`}
      </div>`;
    dom.detailPanel.classList.add("open");
    dom.detailPanel.querySelector("#closeDetailBtn").addEventListener("click", closeDetail);
    dom.detailPanel.querySelector("#favoriteToggleBtn").addEventListener("click", () => toggleFavorite(place.id));
  ...
  function bindPlaceEditor(place) {
    ...
    dom.detailPanel.querySelectorAll("[data-place-field]").forEach(input => {
      input.addEventListener("change", () => {
        ...
        if (field === "name" && !place.images.length) {
          place.photoQuery = nextValue;
          place.photoTriedAt = 0;
          place.photoResolved = false;
        }
        if (!saveDatabase()) {
        ...
        if (field === "type") rerenderEditorAtCurrentPosition();
        showToast("地点资料已保存");
      });
    });
    ...
    dom.detailPanel.querySelector("#autoPhotoBtn")?.addEventListener("click", async () => {
      if (place.images.length && !place.autoCover && !confirm("重新匹配会替换当前照片列表，确定继续吗？")) return;
      const seedCover = SEED?.cities?.[editorCityId]?.places?.find(item => item.id === place.id)?.images?.[0] || "";
      place.images = seedCover ? [seedCover] : [];
      place.image = place.images[0] || "";
      place.autoCover = Boolean(seedCover);
      place.photoAutoDisabled = false;
      place.photoResolved = false;
      place.photoTriedAt = 0;
      saveDatabase();
      rerenderEditorAtCurrentPosition();
      showToast("正在匹配地点实景图…");
      const matched = await resolvePlacePhoto(place, editorCityId);
      saveDatabase();
      if (db.currentCity === editorCityId && selectedId === place.id) rerenderEditorAtCurrentPosition();
      showToast(matched ? "实景图已匹配" : "暂未找到实景图，已保留地点封面");
    });
  ...
  function commitPlaceImages(place, nextImages, message, cityId = db.currentCity) {
    ...
    place.image = place.images[0] || "";
    place.autoCover = false;
    place.photoAutoDisabled = true;
    place.photoResolved = Boolean(place.images.length);
    if (!saveDatabase()) {
      place.images = previousImages;
      place.image = previousImage;
      place.photoAutoDisabled = previousAutoDisabled;
      place.photoResolved = previousResolved;
  ...
  function deletePlace(place) {
    ...
    for (const routePlan of cityData.routePlans) {
      routePlan.route = routePlan.route.filter(id => id !== place.id);
      for (const key of Object.keys(routePlan.segments)) {
        const [startId, endId] = key.split("__");
        if (startId === place.id || endId === place.id) delete routePlan.segments[key];
      }
    }
    syncActiveRouteLegacy(cityData);
    saveDatabase();
    closeDetail();
    renderAll();
    showToast("地点已删除");
  ...
  function toggleFavorite(placeId) {
    ...
    renderBottomNav();
    renderMarkers();
    if (favoriteOnly && !place.favorite) {
      closeDetail();
      showToast("已取消收藏");
      return;
    }
    renderDetail(place);
    showToast(place.favorite ? "已收藏地点" : "已取消收藏");
  }

  function togglePlan(placeId) {
    const cityData = currentCity();
    const routePlan = activeRoutePlan(cityData);
    const existingIndex = routePlan.route.indexOf(placeId);
    if (existingIndex >= 0) {
      routePlan.route = routePlan.route.filter(id => id !== placeId);
      showToast(`已从“${routePlan.name}”移出`);
    } else {
      routePlan.route.push(placeId);
      drawerExpanded = false;
      showToast(`已加入“${routePlan.name}”`);
    }
    syncActiveRouteLegacy(cityData);
    saveDatabase();
    renderMarkers();
  ...
  function getSegment(startId, endId) {
    ...
    if (!isPlainObject(routePlan.segments[key])) {
      ...
      routePlan.segments[key] = { mode: "步行", minutes: Math.max(1, Math.round(km / 4.5 * 60)), km, note: "自动估算，可在编辑模式修改" };
      syncActiveRouteLegacy(cityData);
      saveDatabase();
    }
    const segment = routePlan.segments[key];
  ...
  function renderRoutePlanTabs() {
    const cityData = currentCity();
    dom.routePlanTabs.replaceChildren();
    cityData.routePlans.forEach(routePlan => {
      const button = document.createElement("button");
      const isActive = routePlan.id === cityData.activeRouteId;
      const validCount = routePlan.route.filter(id => cityData.places.some(place => place.id === id)).length;
      button.type = "button";
      button.className = `route-plan-tab${isActive ? " active" : ""}`;
      button.setAttribute("role", "tab");
      button.setAttribute("aria-selected", String(isActive));
      button.innerHTML = `<span>${escapeHtml(routePlan.name)}</span><small>${validCount}</small>`;
      button.addEventListener("click", () => switchRoutePlan(routePlan.id));
      dom.routePlanTabs.appendChild(button);
    });
    dom.deleteRouteBtn.disabled = cityData.routePlans.length <= 1;
  }
  ...
  function renderRouteDrawer() {
    ...
    dom.routeTitle.textContent = `${routePlan.name}（${entries.length}）`;
    const totals = routeTotals(entries);
    if (!entries.length) {
      dom.routeSummary.textContent = drawerExpanded ? "暂无地点，可从地图详情加入当前线路" : "空线路 · 点击展开管理";
    } else if (entries.length > 1) {
      dom.routeSummary.textContent = `${totals.minutes} 分钟交通 · ${totals.kilometres.toFixed(1)} 公里${drawerExpanded ? "" : " · 点击展开"}`;
    } else {
      dom.routeSummary.textContent = drawerExpanded ? "已规划 1 个地点" : "1 个地点 · 点击展开";
    }
    renderRoutePlanTabs();
    ...
    if (!entries.length) {
      ...
      emptyState.className = "route-empty-state";
      emptyState.innerHTML = `<span aria-hidden="true">🗺️</span><div><b>“${escapeHtml(routePlan.name)}”还是空的</b><small>点击地图地点，在详情中选择“加入规划”。</small></div>`;
      dom.timeline.appendChild(emptyState);
    }
  ...
  function switchRoutePlan(routePlanId) {
    const cityData = currentCity();
    if (!cityData.routePlans.some(routePlan => routePlan.id === routePlanId) || cityData.activeRouteId === routePlanId) return;
    cityData.activeRouteId = routePlanId;
    syncActiveRouteLegacy(cityData);
    saveDatabase();
    renderMarkers();
    renderRouteDrawer();
    renderSavedRouteRecords();
    if (selectedId) {
      const place = cityData.places.find(item => item.id === selectedId);
      if (place) renderDetail(place);
    }
    showToast(`已切换到“${activeRoutePlan(cityData).name}”`);
  }

  function addRoutePlan() {
    const cityData = currentCity();
    const defaultName = `线路 ${cityData.routePlans.length + 1}`;
    const input = prompt("请输入新线路名称", defaultName);
    if (input === null) return;
    const name = input.trim() || defaultName;
    const id = `route_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 6)}`;
    cityData.routePlans.push({ id, name, route: [], segments: {} });
    cityData.activeRouteId = id;
    syncActiveRouteLegacy(cityData);
    drawerExpanded = true;
    saveDatabase();
    renderAll();
  ...
  function renameActiveRoutePlan() {
    const cityData = currentCity();
    const routePlan = activeRoutePlan(cityData);
    const input = prompt("修改线路名称", routePlan.name);
    if (input === null) return;
  ...
  function deleteActiveRoutePlan() {
    const cityData = currentCity();
    if (cityData.routePlans.length <= 1) {
      showToast("至少保留一条线路");
      return;
    }
    const routePlan = activeRoutePlan(cityData);
    if (!confirm(`确定删除“${routePlan.name}”吗？线路内的地点不会从地图删除。`)) return;
    const currentIndex = cityData.routePlans.findIndex(item => item.id === routePlan.id);
    cityData.routePlans = cityData.routePlans.filter(item => item.id !== routePlan.id);
    cityData.activeRouteId = cityData.routePlans[Math.max(0, currentIndex - 1)]?.id || cityData.routePlans[0].id;
    syncActiveRouteLegacy(cityData);
    saveDatabase();
    renderAll();
    showToast("线路已删除");
  ...
  function moveRouteEntry(visibleIndex, direction) {
    ...
    const cityData = currentCity();
    const route = activeRoutePlan(cityData).route;
  ...
  function openSettings() {
    ...
    if (dom.settingsDrawer) {
      dom.settingsDrawer.style.transition = "";
      dom.settingsDrawer.style.transform = "";
    }
  ...
  function closeSettings() {
    ...
    if (activeNav === "settings") activeNav = "home";
    renderBottomNav();
  ...
  function renderSettings() {
    ...
    dom.settingsCurrentCityName.textContent = currentCity().name;
  ...
  function routePlanMetrics(cityData, routePlan) {
    const placeById = new Map(cityData.places.map(place => [place.id, place]));
    const places = routePlan.route.map(id => placeById.get(id)).filter(Boolean);
    let minutes = 0;
    let kilometres = 0;
    for (let index = 0; index < places.length - 1; index += 1) {
      const start = places[index];
      const end = places[index + 1];
      const stored = routePlan.segments?.[segmentKey(start.id, end.id)];
      if (isPlainObject(stored)) {
        minutes += Math.max(0, finiteNumber(stored.minutes) ?? 0);
        kilometres += Math.max(0, finiteNumber(stored.km) ?? 0);
      } else {
        const estimatedKm = Math.max(0.1, estimateDistance(start, end) * 1.18);
        kilometres += estimatedKm;
        minutes += Math.max(1, Math.round(estimatedKm / 4.5 * 60));
      }
    }
    return { places, minutes: Math.round(minutes), kilometres: Number(kilometres.toFixed(1)) };
  }

  function renderSavedRouteRecords() {
    const cityData = currentCity();
    dom.savedRouteList.replaceChildren();
    cityData.routePlans.forEach((routePlan, index) => {
      const metrics = routePlanMetrics(cityData, routePlan);
      const isActive = routePlan.id === cityData.activeRouteId;
      const card = document.createElement("article");
      card.className = `saved-route-card${isActive ? " active" : ""}`;
      const preview = metrics.places.slice(0, 3).map(place => place.name).join(" · ") || "还没有加入地点";
      const extra = Math.max(0, metrics.places.length - 3);
      card.innerHTML = `<div class="saved-route-index" aria-hidden="true">${String(index + 1).padStart(2, "0")}</div><div class="saved-route-copy"><div class="saved-route-title-row"><b>${escapeHtml(routePlan.name)}</b>${isActive ? '<span class="saved-route-active">当前</span>' : ""}</div><div class="saved-route-stats"><span>${metrics.places.length} 个地点</span><span>${metrics.minutes} 分钟</span><span>${metrics.kilometres} km</span></div><p>${escapeHtml(preview)}${extra ? ` 等 ${extra + 3} 站` : ""}</p></div><button class="saved-route-open" type="button">${isActive ? "查看" : "打开"}<span aria-hidden="true">›</span></button>`;
      const openRoute = () => openSavedRoute(routePlan.id);
      card.tabIndex = 0;
      card.setAttribute("role", "button");
      card.setAttribute("aria-label", `${isActive ? "查看" : "打开"}${routePlan.name}`);
      ...
      dom.savedRouteList.appendChild(card);
    });
  }

  function openSavedRoute(routePlanId) {
    const cityData = currentCity();
    const routePlan = cityData.routePlans.find(item => item.id === routePlanId);
    if (!routePlan) return;
    cityData.activeRouteId = routePlan.id;
    drawerExpanded = true;
    syncActiveRouteLegacy(cityData);
    saveDatabase();
    renderMarkers();
  ...
  function bindSettingsEdgeSwipe() {
    if (!dom.settingsDrawer) return;
    const edgeWidth = 28;
    ...
    const resetDrawerPosition = withAnimation => {
      dom.settingsDrawer.style.transition = withAnimation ? "transform 0.18s ease" : "";
      dom.settingsDrawer.style.transform = "translateX(0px)";
      if (withAnimation) {
        clearTimeout(dom.settingsDrawer._swipeResetTimer);
        dom.settingsDrawer._swipeResetTimer = setTimeout(() => {
          dom.settingsDrawer.style.transition = "";
          dom.settingsDrawer.style.transform = "";
        }, 220);
      }
    ...
    const finish = shouldClose => {
      ...
      pointerId = null;
      if (shouldClose) {
        dom.settingsDrawer.style.transition = "transform 0.18s ease";
        dom.settingsDrawer.style.transform = `translateX(${Math.max(latestDx, dom.settingsDrawer.getBoundingClientRect().width)}px)`;
        setTimeout(() => {
          closeSettings();
        }, 160);
      } else {
        resetDrawerPosition(true);
      }
    };

    dom.settingsDrawer.addEventListener("pointerdown", event => {
      if (!dom.settings.classList.contains("open")) return;
      if (event.pointerType === "mouse" && event.button !== 0) return;
      const rect = dom.settingsDrawer.getBoundingClientRect();
      if (event.clientX - rect.left > edgeWidth) return;
      tracking = true;
      ...
      intentLocked = false;
      isHorizontal = false;
      dom.settingsDrawer.style.transition = "none";
      try { dom.settingsDrawer.setPointerCapture?.(event.pointerId); } catch (_) {}
    });

    dom.settingsDrawer.addEventListener("pointermove", event => {
      ...
      if (!intentLocked) {
        ...
        intentLocked = true;
        isHorizontal = Math.abs(dx) > Math.abs(dy) * 1.15;
      }
      if (!isHorizontal) {
        finish(false);
        return;
      ...
      latestDx = dx;
      event.preventDefault();
      dom.settingsDrawer.style.transform = `translateX(${dx}px)`;
    });

    const endSwipe = event => {
      if (!tracking || event.pointerId !== pointerId) return;
      const rect = dom.settingsDrawer.getBoundingClientRect();
      const threshold = Math.min(Math.max(closeThreshold, rect.width * 0.18), rect.width * 0.4);
      finish(isHorizontal && latestDx >= threshold);
    };

    dom.settingsDrawer.addEventListener("pointerup", endSwipe);
    dom.settingsDrawer.addEventListener("pointercancel", () => finish(false));
  }
  ...
  function bindEvents() {
    ...
    dom.currentCityBtn?.addEventListener("click", event => {
      event.stopPropagation();
      setSearchExpanded(false);
      setCityPopover(!dom.cityPopover.classList.contains("open"));
    });
    dom.searchToggle.addEventListener("click", event => {
      event.stopPropagation();
      activeNav = "places";
      renderBottomNav();
      setSearchExpanded(!dom.searchShell.classList.contains("open"));
    });
    dom.bottomNav?.addEventListener("click", event => {
      const button = event.target.closest("[data-nav]");
      if (button) handleBottomNavigation(button.dataset.nav);
    });
    dom.addRouteBtn.addEventListener("click", event => {
      event.stopPropagation();
      addRoutePlan();
    });
    dom.settingsAddRouteBtn.addEventListener("click", addRoutePlan);
    ...
    window.addEventListener("keydown", event => {
      ...
      if (dom.searchShell.classList.contains("open")) setSearchExpanded(false);
      else if (dom.addPlaceModal.classList.contains("open")) cancelAddPlace(false);
